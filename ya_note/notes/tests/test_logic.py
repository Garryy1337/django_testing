...
class NoteCreationTest(TestCase):
    ...
    def test_create_note_with_empty_title(self):
        """
        Проверка, что заметка не создается, если не указано название.
        """
        response = self.client.post(reverse('notes:add'), data={
            'title': '',
            'text': 'Текст заметки',
    ...
    def test_create_note_with_empty_text(self):
        """
        Проверка, что заметка не создается, если не указан текст.
        """
        response = self.client.post(reverse('notes:add'), data={
            'title': 'Название заметки',
            'text': '',
...
class NoteSlugTest(TestCase):
    def test_duplicate_slug_not_allowed(self):
        ...
        note1 = Note.objects.create(title='Заметка 1', text='Текст заметки 1',
                                    author=user)
        note2 = Note(title='Заметка 2', text='Текст заметки 2', author=user)
        note2.save()

        self.assertNotEqual(note1.slug,
                            note2.slug)
...
class NoteSlugGenerationTest(TestCase):
    def test_slug_auto_generation(self):
        """
        Проверка, что slug формируется автоматически, если не заполнен при
        создании заметки.
        """
        user = User.objects.create_user(username='testuser',
                                        password='testpassword')
...
class NoteAuthorizationTest(TestCase):
    def setUp(self):
        ...
        self.note = Note.objects.create(title='Заметка', text='Текст заметки',
                                        author=self.user1)
    ...
    def test_user_can_edit_own_note(self):
        ...
        response = self.client.post(
            reverse('notes:edit', args=[self.note.slug]), data={
    ...
    def test_user_cannot_edit_other_user_note(self):
        ...
        self.assertEqual(response.status_code,
                         404)
        self.note.refresh_from_db()
        self.assertNotEqual(self.note.title,
                            'Новое название')
